...
async fn process_manager_loop(
    ...
    loop {
        ...
        let _ = sender
            .send(AppEvent::Notify(Notification::info(format!(
                "Started {}: '{}'",
                ...
                cmd = receiver.recv() => {
                    ...
                    match event {
                        ...
                        ProcessEvent::Stop => {
                            ...
                            io_task.abort();
                            let _ = sender
                                .send(AppEvent::Notify(Notification::warning(format!(
                                    "Killed process (pid: {:?})",
                                    pid
                                ))))
                                .await;
...
async fn io_loop(stdout: ChildStdout, stderr: ChildStderr, sender: Sender<AppEvent>) {
